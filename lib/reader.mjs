...
export function readGlobalConfig() {
  ...
  const settingsLocal = readJSON(join(CLAUDE_DIR, 'settings.local.json'));
  ...
  // Collect MCP servers from both scopes
  const globalMcp = [];
  ...
  for (const [name, cfg] of Object.entries(settingsLocal?.mcpServers ?? {})) {
    // local overrides global — skip duplicates, tag as local
    if (!globalMcp.some(m => m.name === name && m.scope === 'global-local')) {
      globalMcp.push({ name, scope: 'global-local', ...cfg });
    }
  }
...
export function aggregateAll(projects) {
  ...
  const projectConfigs = projects.map((p) => readProjectConfig(p.path)).filter(Boolean);
...
export function togglePlugin({ pluginId, scope, projectPath, enabled }) {
  try {
    if (scope === 'user') {
      ...
      const filePath = join(CLAUDE_DIR, 'settings.json');
