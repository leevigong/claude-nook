...
import { readGlobalConfig, readProjectConfig, aggregateAll, togglePlugin, getKnownPluginIds, movePermission, moveMcpServer, moveHook, deleteMcpServer } from './reader.mjs';
import { hideProject, unhideProject, readHiddenList } from './hidden.mjs';
...
// Two caches: `cachedAllProjects` is the full scanner output including hidden
// entries (used to validate paths for hide/unhide); `cachedProjects` is the
// visible subset shown in the sidebar. Clearing `cachedAllProjects` also
// invalidates the visible cache.
let cachedAllProjects = null;
let cachedProjects = null;

function getAllProjects() {
  if (!cachedAllProjects) {
    cachedAllProjects = scanProjects({ includeHidden: true });
  }
...
function getProjects() {
  if (!cachedProjects) {
    cachedProjects = getAllProjects().filter((p) => !p.hidden);
  }
...
function makeHandler(serverPort) {
  ...
  // API routes first
  // API: Full overview
  // Includes every known project (hidden ones too) so the UI can render a
  // "Show hidden" toggle without a second round-trip. Each project carries
  // `hidden: true/false` and a `sources` array describing why it was
  // detected.
  if (pathname === '/api/overview' && method === 'GET') {
    const projects = getAllProjects();
    const data = aggregateAll(projects);
    data.hiddenProjectPaths = readHiddenList();
  ...
  if (pathname.startsWith('/api/projects/') && method === 'GET') {
    ...
    // Only allow projects discovered by the scanner (including hidden ones —
    // hiding is a display-layer concern, not an access gate).
    const knownProjects = getAllProjects();
  ...
  // API: Hide a project from the sidebar. Validates against the full
  // (includeHidden) project list so the user can't hide arbitrary paths.
  if (pathname === '/api/projects/hide' && method === 'POST') {
    let parsed;
    try {
      parsed = await parseJsonBody(req);
    } catch (e) {
    ...
    if (typeof projectPath !== 'string' || !projectPath) {
      return json(res, { ok: false, error: 'Invalid projectPath' }, 400);
    }
    const all = getAllProjects();
    if (!all.some((p) => p.path === projectPath)) {
      return json(res, { ok: false, error: 'Project not allowed' }, 403);
    }
    try {
  ...
  // API: Unhide a project. Same path validation as /hide.
  if (pathname === '/api/projects/unhide' && method === 'POST') {
    let parsed;
    try {
      parsed = await parseJsonBody(req);
    } catch (e) {
  ...
  // --- Scope move/delete endpoints ---
  // Common validation: scopes ∈ {user, project, local}; projectPath must be a known project
  // when either side is non-user. .mcp.json is intentionally not handled here.
  function validateMoveBasics(parsed) {
    const { from, to, projectPath } = parsed || {};
    if (!['user', 'project', 'local'].includes(from)) return 'Invalid from scope';
    if (!['user', 'project', 'local'].includes(to)) return 'Invalid to scope';
    if (from === to) return 'Source and target must differ';
    if (from !== 'user' || to !== 'user') {
      // Allow moves on hidden projects too — hidden is just a sidebar preference.
      const knownProjects = getAllProjects();
      if (!knownProjects.some((p) => p.path === projectPath)) {
  ...
  if (pathname === '/api/move/permission' && method === 'POST') {
    let parsed;
    try { parsed = await parseJsonBody(req); } catch (e) { return json(res, { ok: false, error: e.message }, 400); }
    const err = validateMoveBasics(parsed);
    if (err) return json(res, { ok: false, error: err }, 400);
    const { from, to, projectPath, policy, rule } = parsed;
    return json(res, movePermission({ from, to, projectPath, policy, rule }));
  }

  if (pathname === '/api/move/mcp' && method === 'POST') {
    let parsed;
    try { parsed = await parseJsonBody(req); } catch (e) { return json(res, { ok: false, error: e.message }, 400); }
    const err = validateMoveBasics(parsed);
    if (err) return json(res, { ok: false, error: err }, 400);
    const { from, to, projectPath, name, sourceFile } = parsed;
    return json(res, moveMcpServer({ from, to, projectPath, name, sourceFile }));
  }

  if (pathname === '/api/delete/mcp' && method === 'POST') {
    let parsed;
    try { parsed = await parseJsonBody(req); } catch (e) { return json(res, { ok: false, error: e.message }, 400); }
    const { fromScope, projectPath, name, sourceFile } = parsed || {};
    if (!['user', 'project', 'local'].includes(fromScope)) {
      return json(res, { ok: false, error: 'Invalid scope' }, 400);
    }
    if (fromScope !== 'user') {
      const knownProjects = getAllProjects();
    ...
    return json(res, deleteMcpServer({ fromScope, projectPath, name, sourceFile }));
  }

  if (pathname === '/api/move/hook' && method === 'POST') {
    let parsed;
    try { parsed = await parseJsonBody(req); } catch (e) { return json(res, { ok: false, error: e.message }, 400); }
    const err = validateMoveBasics(parsed);
    if (err) return json(res, { ok: false, error: err }, 400);
    const { from, to, projectPath, event, matcher, command, type } = parsed;
    return json(res, moveHook({ from, to, projectPath, event, matcher, command, type }));
  }
