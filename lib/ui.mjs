  ...
  <script>
        ...
        html += \`<div class="table-wrap" style="border-color:var(--accent)">
          ...
          <table>
            ...
            \${Object.entries(hookEvents).map(([evt, cfg]) => \`<tr>
              <td><span class="tag tag-teal">\${esc(evt)}</span></td>
    ...
    function renderAllHooks() {
      ...
      html += \`<div class="table-wrap">
        ...
        <table>
          <tr><th>Event</th><th style="width:340px">Command</th><th>Scope</th><th>Project</th></tr>
          \${allItems.map(h => {
            const scopeColor = h.scope === 'user' ? 'purple' : h.scope === 'project' ? 'blue' : 'orange';
            const cmd = h.command || '-';
            const display = cmd.length > 50 ? cmd.slice(0, 50) + '…' : cmd;
            return \`<tr>
              <td><span class="tag tag-teal">\${esc(h.event)}</span></td>
              <td style="font-family:monospace;font-size:12px;max-width:340px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="\${esc(cmd)}">\${esc(display)}</td>
