    ...
    function renderPermissions(sources, opts = {}) {
      ...
      const sectionHTML = (policy) => {
        ...
        return \`<div class="table-wrap perm-section" data-policy="\${policy}" style="margin-bottom:16px">
          ...
          <table>
            \${colHead}
            \${items.map(r => \`<tr class="perm-row" data-rule="\${esc(r.rule.toLowerCase())}" data-project="\${esc((r.project || '').toLowerCase())}">
              <td style="font-family:monospace;font-size:12px;word-break:break-all">\${esc(r.rule)}</td>
              <td>\${scopeTag(r.scope)}</td>
              \${includeProject ? \`<td style="font-size:12px;color:var(--text-muted)">\${esc(r.project || '')}</td>\` : ''}
            </tr>\`).join('')}
          </table>
        </div>\`;
      };
