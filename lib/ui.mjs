      ...
      switch (activeTab) {
        ...
        case 'permissions': {
          ...
          const userPermsHtml = userCount
            ? renderPermissions([{ scope: 'user', settings: appData.global.settings }])
        ...
        case 'mcp': {
          ...
          const mcpContent = (servers) => servers.length
            ? \`<div class="table-wrap">\${mcpTable(servers)}</div>\`
          ...
          // Combined env vars block
          const allServers = [...userMcp, ...localMcp];
          const serversWithEnv = allServers.filter(s => s.env && Object.keys(s.env).length);
