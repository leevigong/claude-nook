...
<body>
  ...
  <script>
    ...
    function renderAllPermissions() {
      ...
      return \`<div class="page-title">All Permissions</div>
        <div style="font-size:13px;color:var(--text-muted);margin-bottom:16px;padding:10px 14px;background:var(--bg-hover);border-radius:var(--radius-sm)">우선순위: <strong>Deny</strong> &gt; <strong>Ask</strong> &gt; <strong>Allow</strong></div>
      \${body}\`;
    }
          ...
          if (globalHookItems.length) {
            html += \`<details style="margin-bottom:16px">
              <summary style="cursor:pointer;user-select:none;padding:8px 4px;font-size:13px;font-weight:600;color:var(--text-dim)">
                Global Hooks (\${globalHookItems.length})
              </summary>
              <div class="table-wrap" style="margin-top:8px">
                <table style="opacity:0.7">
                  ...
                  \${globalHookItems.map(hookRow).join('')}
                </table>
              </div>
            </details>\`;
          }
          if (projectHookItems.length) {
            html += \`<div class="table-wrap" style="margin-top:16px">
              <div class="table-title">Project Hooks (\${projectHookItems.length})</div>
              <table>
                <tr><th style="width:130px">Event</th><th style="width:120px">Matcher</th><th>Command</th><th style="width:80px">Scope</th></tr>
                \${projectHookItems.map(hookRow).join('')}
              </table>
            </div>\`;
          } else {
            html += '<div class="empty">No project hooks configured</div>';
          }
          break;
        }
        case 'mcp': {
